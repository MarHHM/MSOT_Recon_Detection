function [ Recon, A_mat, sigMat_filtered, return_code, return_message ] = evalFilters_reconWrapper( A_mat, sigMat, imp_resp, n, proj, r_sensor, angle_sensor, c, image_select, fs, limits, time_res, filter_f, lsqr_iter, varargin )
%% INPUT
...
%         firstNNZ = firstNNZ +500 ;
%         cut = ceil( .05 * size( sigMat, 1 ) ) ;
        cut = 50;
        sigMat = sigMat - ones( size( sigMat, 1 ), 1 ) * sigMat( cut+1, : ) ;
        sigMat( [1:cut ], : ) = 0 ;
%         sigMat( 1:firstNNZ-1, : ) = 0 ;
%         sigMat( 1:firstNNZ-1, : ) = ones(firstNNZ-1,1)*mean(sigMat(firstNNZ:firstNNZ+10,:)) ;
%         sigMat( firstNNZ:end, : ) = sigMat( firstNNZ:end, : ) - ones( length( sigMat( firstNNZ:end, 1 ) ), 1 ) * sigMat( firstNNZ, : ) ;
%     end ;
