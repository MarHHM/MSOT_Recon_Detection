...
lastslash = fstr.lastIndexOf( filesep );
if lastslash < 0,
    dirname = ['.' filesep];
else
...
xi = strfind(dirname,filesep);
if numel(xi)>1, 
    datainfo.StudyPath = dirname(1:xi(end)-1); 
    datainfo.StudyName = dirname(xi(end-1)+1:xi(end)-1); 
else
    datainfo.StudyPath = '.';
    datainfo.StudyName = '<unknown>';
end;
...
datainfo.USpresent = false;
datainfo.US.n = 0;
try
    datainfo.USpresent = logical(md.getULTRASOUNDPRESENT);
    datainfo.US.n = uint16(md.getULTRASOUNDRESOLUTION);
end
...
datainfo.OAMPreset = struct;
datainfo.OAMPreset.Name = '';
datainfo.OAMPreset.roi = [];
try
    oap = dm.getOAMPreset;
    datainfo.OAMPreset.Name = char(oap.getName);
    
    rpa = dm.getStudyPreset.getReconPresets.getDataModelReconPreset.getSystemReconPresets;
    rp = rpa(1).getReconstructionPreset;
    datainfo.OAMPreset.roi = double(rp.getRoiHigh) - double(rp.getRoiLow);
end
...
datainfo.HWDesc.LightSpotSize = 0;
datainfo.HWDesc.AxialOffset = 0;
datainfo.HWDesc.CouplantSOS = 0;
datainfo.HWDesc.CouplantCOR = 0;
datainfo.HWDesc.CouplantCurvature = 0;
try
    datainfo.HWDesc.LightSpotSize = double(hw.getLIGHTSPOTSIZE);
    datainfo.HWDesc.AxialOffset = double(hw.getAXIALOFFSET);
    datainfo.HWDesc.CouplantSOS = double(hw.getCOUPLANTSPEEDOFSOUND);
    datainfo.HWDesc.CouplantCOR = double(hw.getCOUPLANTCENTEROFROTATION);
    datainfo.HWDesc.CouplantCurvature = double(hw.getCOUPLANTCURVATURERADIUS);
end
...
datainfo.HWDesc.DeviceSN = '';
datainfo.HWDesc.TransducerSN = '';
datainfo.HWDesc.DAQAddress = '';
datainfo.MeasurementDesc.SWVersion = '';
datainfo.MeasurementDesc.TrimSOS = 0;
datainfo.MeasurementDesc.CouplantCorr = false;
try
    datainfo.HWDesc.DeviceSN = char(sn.getDeviceSN);
    datainfo.HWDesc.TransducerSN = char(sn.getTransducerSN);
    datainfo.HWDesc.DAQAddress = char(sn.getDAQMACaddress);
    datainfo.MeasurementDesc.SWVersion = char(sn.getSWVersion);
    datainfo.MeasurementDesc.TrimSOS = double(sn.getTrimSpeedOfSound);
    datainfo.MeasurementDesc.CouplantCorr = logical(sn.getCouplantCorrection);
end
...
datainfo.ScanFrames(i).USoffset = nan;
datainfo.US.timestamps = [];
datainfo.US.OAframes = [];
datainfo.US.OArep = [];
...
for i = 1:length(fa)
    ...
    if ~isempty(fr.getTEMPERATURE); datainfo.ScanFrames(i).Temperature = fr.getTEMPERATURE.getDoubleValue; else; datainfo.ScanFrames(i).Temperature = 0; end;  
    if ~isempty(fr.getWAVELENGTH); datainfo.ScanFrames(i).Wavelength = fr.getWAVELENGTH.getIntValue; end;
    if ~isempty(fr.getPOWER); datainfo.ScanFrames(i).CorrectionFactor = fr.getPOWER.getDoubleValue; end;
    if ~isempty(fr.getZPOS); datainfo.ScanFrames(i).ZPos =  single(fr.getZPOS.getDoubleValue); else; datainfo.ScanFrames(i).ZPos = 0; end;
    ...
    try
        uoff = uint16(fr.getUltraSoundFrameOffset);
        datainfo.ScanFrames(i).USoffset = uoff;
        if numel(datainfo.US.timestamps) < uoff+1, 
            datainfo.US.timestamps(uoff+1) = datainfo.ScanFrames(i).RelTime;
            datainfo.US.OAframes(uoff+1) = i;
            datainfo.US.OArep(uoff+1) = uint16(fr.getREPETITION);
        end;
    end
...
zpos = round(zpos*10)/10;
datainfo.RepNum = maxRep;
datainfo.RunNum = maxRun; 
datainfo.ZPositions = zpos;
...
if nestframes < numel(datainfo.ScanFrames),
   ...
   nzpdiff = ceil((numel(datainfo.ScanFrames) - nestframes)/numel(datainfo.Wavelengths)/numshots);
   nzp = nzp + nzpdiff;
end
...
tpm = nan(datainfo.RunNum,nzp,datainfo.RepNum,numel(datainfo.Wavelengths),numshots,'double');
...
for i = 1:numel(datainfo.ScanFrames)
    ...
    zptmp = round(datainfo.ScanFrames(i).ZPos*10)/10;
    zp = find(datainfo.ZPositions == zptmp);
    if isempty(zp); [val ind]=min(abs(datainfo.ZPositions - zptmp)); zptmp = datainfo.ZPositions(ind); zp = find(datainfo.ZPositions == zptmp); end;
    ...
    tpm(ru,zp,re,wl,sh) = datainfo.ScanFrames(i).Temperature;
end
datainfo.ScanStructure = idm;
datainfo.RelTime = tsm;
datainfo.LaserEnergy = lpm;
datainfo.Temperature = tpm;
...
%% Load View Settings (if exists)
xfn = strrep(datainfo.FileName,'.bin','.img');
datainfo.ImgFileName = [];
datainfo.ImgSession = [];

waitbar(0,wbar,'Loading Imaging Session');
if exist(xfn,'file'),
    datainfo.ImgFileName = xfn;
    datainfo.ImgSession = loadMSOTView(xfn,datainfo);
end



%% decide if exit
if nargin >= 2 && strcmp(varargin{2},'skipimg'),
    close(wbar);
    return;
end
...
for mn = 1:length(mna)
    ...
    for mfr = 1:numel(mfra)
        ...
        if rn > 0,
            ...
            rframes = datainfo.ReconNode(rn).IDLookup(datainfo.MSPNode(mn).Slices(mfr).ReconFrames(:)+1);  % all associated recon frames
            sframeid = datainfo.ReconNode(rn).Frames(rframes(1)).ScanFrames(1)+1; % first scan frame for referencing
            datainfo.MSPNode(mn).Slices(mfr).ReconFrameNumbers = rframes;
            datainfo.MSPNode(mn).Slices(mfr).ScanFrames = cell2mat({datainfo.ReconNode(rn).Frames(rframes).ScanFrames});
            datainfo.MSPNode(mn).Slices(mfr).ZPos = datainfo.ScanFrames(sframeid).ZPos;
            datainfo.MSPNode(mn).Slices(mfr).ZNum = datainfo.ScanFrames(sframeid).ZNum;
            datainfo.MSPNode(mn).Slices(mfr).Run = datainfo.ScanFrames(sframeid).Run;
            datainfo.MSPNode(mn).Slices(mfr).Repetition = datainfo.ScanFrames(sframeid).Repetition;
            datainfo.MSPNode(mn).Slices(mfr).RelTime = datainfo.ScanFrames(sframeid).RelTime;
