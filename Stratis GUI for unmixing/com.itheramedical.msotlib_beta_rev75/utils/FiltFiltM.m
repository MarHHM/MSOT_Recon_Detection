function Y = FiltFiltM(b, a, X, Dim, numCores)
...
if hasFilterX  % Use the faster C-mex filter function: -------------------------
   ...
   [dum, Zi] = FilterX(b, a, Xi, IC * Xi(1, :), 'forward');   %#ok<ASGLU>
   
   % Use the final conditions of the initial part for the actual signal:
   [Ys, Zs]  = FilterX(b, a, X,  Zi, 'forward');              % "s"teady state
   Yf        = FilterX(b, a, Xf, Zs, 'forward');              % "f"inal conditions
