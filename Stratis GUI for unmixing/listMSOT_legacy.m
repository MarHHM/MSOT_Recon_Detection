...
% studydir = 'C:\Users\Saturn\Documents\MATLAB\GUI\data';

%% file list
wbar = waitbar(0,'Initialising...');

%From here on...
cfiles = rdir([studydir '\**\' '*.msot']);
...
for ind = 1:numel(cfiles)
    ...
    datainfoarr(ind).Comment = char(dm.getScanNode.getComment);
    datainfoarr(ind).RealPath = dirname;
    datainfoarr(ind).Complete = false;
    ...
    datainfoarr(ind).USpresent = false;
    try
        datainfoarr(ind).USpresent = logical(md.getULTRASOUNDPRESENT);
    end
