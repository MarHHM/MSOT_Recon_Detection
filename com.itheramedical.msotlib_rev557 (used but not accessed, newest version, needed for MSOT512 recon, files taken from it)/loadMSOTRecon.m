...
% image return vector
n = double(datainfo.ReconNode(rNode).Resolution);
if datainfo.is3D,
    nz = datainfo.ReconNode(rNode).ZResolution;
    R = nan([n n nz prod(svec)],'double');
else
...
for j = 1:numel(selMat)
    ...
    try
        if datainfo.is3D,
            startbyte = (id-1)*n*n*8;    %determine start position (double prec), one value is counted for each z slice
            fseek(FID,startbyte,-1);     % move towards position
            for zi = 1:nz,
                R(:,:,zi,j) = fread(FID,[n n],'double');
            end
        else
