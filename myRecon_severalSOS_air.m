...
sos_vec = linspace(1480,1540,21);
% sos_vec = linspace(1480,1540,80);
...
MB_MAT_FOLDER = 'D:\Marwan\MB_matrices';

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%% load sigMat to read dims & params
if ~exist('sigMat', 'var')
    [datainfo, sigMat, sigMat_pathName] = loadSigMat_iThera('D:\Marwan\Datasets_offline\(2016-12-01) central rod with uspheres around\no avg per acqstn\air only');
else
...
for i = 2:length(sos_vec)
    ...
    if SAVE_RECON
        save(strcat(sigMat_pathName, 'recons\recon', RECON_METHOD, '_imSz', num2str(n), '_sos', num2str(c), '_rep', num2str(rep_begin), 'to', num2str(rep_end), '.mat'), 'reconStack');
    end
    delete(msgbox_h);
end
%% show specific im
