...
% params
n = 200;        % im width (pixels)
image_width = 20e-3;        % im width (m)
filter_min = 0.02e6;
filter_max = 7e6;
RECON_METHOD = 'MB_Tik';        % 'BackProjection'  'MB_Tik'  'MB_TVL1'
NONNEG = 1;
time_res = 2;                                       % time resolution for model-based (bigger -> finer res but bigger A & more recon time)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
% loading into RAM (if not the same last data)
[FileNamesigMat, temp, FilterIndexsigMat] = uigetfile({'*.msot'}, 'mytitle', 'C:\Datasets_offline\Marwan_1\Scan_6');
...
% limit #reps due to memory limitiation of prefilt fun
Zpos_begin = 1;
...
for run_idx = 1:1
    for slc_idx = Zpos_begin : Zpos_end
        for wl_idx = wl_begin : wl_end
            for rep_idx = rep_begin : rep_end
...
save(strcat(PathNamesigMat, 'recon', RECON_METHOD, '_rep', num2str(rep_begin), 'to', num2str(rep_end)), 'ReconStack');
