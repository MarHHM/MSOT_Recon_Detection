function [overlay] = overlay_multipurpose(sat, im, background, Mask, map)

% sat: functional image (cHb, cHbO2 or sO2)
% im, background: anatomical image (OA recon)
% mask: where u want to overlay only
% map: desired colormap 
...
%     figure; imagesc(sat)
    rgb_SAT = ind2rgb(uint8(ceil(64*sat)), map);
...
%         rgb_SAT(:,:,i) = rgb_SAT(:,:,i).*(background);
    end
