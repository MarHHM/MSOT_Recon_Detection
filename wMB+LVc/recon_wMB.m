function recon_wMB(scan_path, nonNeg, LVc, RECON_ALL)
...
parent_convMB_recon = load([scan_path '\recons\MB_Tik - nonNeg_' int2str(nonNeg) '-zpos_1-reps_1-wls_1-reconRes_7.5e-05-imW_0.03.mat']);      % load an aribtrary recon to test the masks
disp('done.')
...
% end
imToChooseMask = parent_convMB_recon.Recon_MB(:, :, 1, 1, 1, 1);   % the slice shown to draw the mask (in case LOAD_A_DIMS = 0)
...
figure('name', 'just to confirm that masks are correct!!');
    imagesc( imToChooseMask ), title( 'conv MB - arbitrary slice', 'interpreter', 'none' ), colormap( bone ), colorbar, axis image off;
...
%                         y_sensor = r_sensor*sin(theta);                     % vertical position of the transducer
                        ...
                        if LVc            % access detectors in red part (partial(D_u) in fig. 2a) & replace the latter part of A_ij(j) by the const A_ij(j_c) (i.e. fig. 2c)
...
figure,...
    imagesc(ReconW(:, :, 1, 1, 1, 1)); colormap(bone), title([LUIS_WEIGHT_METHOD ' (\omega = ' num2str(w) ') - LVc = ' num2str(LVc)]), colorbar, axis image off;    % OR is f(t_{ij})
...
if SAVE_RECON
    ...
    reconStruct__fname = strcat( scan_path, '\recons\', LUIS_WEIGHT_METHOD, ' - nonNeg_', num2str(nonNeg), '-zpos_',...
                                num2str(zpos_end-zpos_begin+1), '-reps_', num2str(rep_end-rep_begin+1), '-wls_', num2str(wl_end-wl_begin+1),...
                                '-reconRes_', num2str(reconRes), '-imW_', num2str(im_w) , '-w_', num2str(w),'-LVc_', num2str(LVc), '.mat');
