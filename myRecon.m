...
for i = 1:length(sos_vec)
    disp(['currently at sos ' num2str(i) ' of 20']);
    ...
    if SAVE_RECON
        save(strcat(PathNamesigMat, 'recon', RECON_METHOD, '_imSz', num2str(n), '_sos', num2str(c), '_rep', num2str(rep_begin), 'to', num2str(rep_end)), 'reconStack');
    end
end
%% show specific im
% slcToShow = 1;
